...
func init() {
	...
	// Server Logging
	if _, err := os.Stat(filepath.Join(core.CurrentDir, "data", "log", "tulliusServerLog.txt")); os.IsNotExist(err) {
		errM := os.MkdirAll(filepath.Join(core.CurrentDir, "data", "log"), 0750)
		if errM != nil {
			message("warn", "there was an error creating the log directory")
		}
		serverLog, errC := os.Create(filepath.Join(core.CurrentDir, "data", "log", "tulliusServerLog.txt"))
		if errC != nil {
			message("warn", "there was an error creating the tulliusServerLog.txt file")
			return
		}
		...
		if core.Debug {
			message("debug", fmt.Sprintf("Created server log file at: %s\\data\\log\\tulliusServerLog.txt", core.CurrentDir))
		}
	}

	var errLog error
	serverLog, errLog = os.OpenFile(filepath.Join(core.CurrentDir, "data", "log", "tulliusServerLog.txt"), os.O_APPEND|os.O_WRONLY, 0600)
	if errLog != nil {
		message("warn", "there was an error with the tullius Server log file")
...
func Server(logMessage string) {
	...
	if err != nil {
		message("warn", "there was an error writing to the tullius Server log file")
